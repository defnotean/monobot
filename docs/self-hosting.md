...
- **Vision works locally, with an optional accuracy fallback.** The local-vision adapter (`packages/shared/src/ai/localVision.js`) is wired into both bots' live message paths and speaks native Ollama (`/api/chat` with base64 images). Set `OLLAMA_VISION_URL=http://localhost:11434` and optionally `OLLAMA_VISION_MODEL` (default `moondream`; use `qwen2.5vl:7b` only when it stays on GPU). `LOCAL_VISION_CLOUD_FALLBACK=weak` retries vague or failed local reads with Gemini; set it to `off` for strict local-only image handling.
