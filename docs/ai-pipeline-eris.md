...
2. **Gating** — a gauntlet of cheap skip-checks decides *whether to reply at all*. Most messages stop here, before spending a single AI token.
3. **Context building** — assemble the system prompt: personality, mood, memory, history, and which tools to offer.
