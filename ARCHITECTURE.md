...
**Tier model.** `ai/toolRegistry.js` registers ~10 categories with keyword-regex patterns. Tools always sent to the model land in `_alwaysInclude` (memory, web, GIF, notes, mood); the matched-category tools are added to Tier 1; the rest become a Tier 2 name-only catalog appended to the prompt — the executor still dispatches by name regardless. Eris currently uses this two-tier path. Irene's registry is wired but `messageCreate.js` sends all tools every message today; the registry is tracking usage for when Tier 2 is re-enabled.
