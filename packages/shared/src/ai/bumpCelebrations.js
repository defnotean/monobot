...
// @ts-nocheck -- shared factory binds untyped Discord/database deps; bot suites cover behavior.
...
function _isoWeekKeyShared(d = new Date()) {
...
/**
 * Count bumps for a guild since `sinceMs`. Uses eris_bumps / irene_bumps
 * transparently — the analytics module already handles which table.
 */
/**
 * Build bump celebration helpers for one bot.
 *
 * @param {object} deps
 ...
 * @param {(userId: string, botName: string) => Promise<any>} deps.getUserPrefs
 * @param {(msg: string) => void} [deps.log]
 * @param {string} [deps.defaultBumpsTable]
 * @param {string} [deps.defaultBotName]
 */
export function createBumpCelebrations({
  getGuildSettings,
  setGuildSetting,
  getSupabase,
  getBumpAnalytics,
  ...
  if (typeof getGuildSettings !== "function") {
    throw new Error("createBumpCelebrations: getGuildSettings function is required");
  }
  if (typeof setGuildSetting !== "function") {
    throw new Error("createBumpCelebrations: setGuildSetting function is required");
  }
  if (typeof getSupabase !== "function") {
    throw new Error("createBumpCelebrations: getSupabase function is required");
  }
  if (typeof getBumpAnalytics !== "function") {
    throw new Error("createBumpCelebrations: getBumpAnalytics function is required");
  }
  if (typeof getUserPrefs !== "function") {
    throw new Error("createBumpCelebrations: getUserPrefs function is required");
  }

async function countBumpsSince(guildId, sinceMs, bumpsTable = defaultBumpsTable) {
  try {
 ...
 */
async function maybeCelebrateBumpathon(client, guildId, bumpsTable = defaultBumpsTable) {
 ...
 */
function startBumpathonWatcher(client, { intervalMs = 5 * 60_000, bumpsTable = defaultBumpsTable } = {}) {
  const tick = async () => {
 ...
 */
async function maybeCelebrateStreakMilestone(client, { guildId, service = "disboard" }) {
  try {
    const { getGuildStreak } = await getBumpAnalytics();
 ...
 */
async function recordStreakBaseline({ guildId, service = "disboard" }) {
  try {
    const { getGuildStreak } = await getBumpAnalytics();
    const streak = await getGuildStreak(guildId, service);
    const settings = getGuildSettings(guildId) || {};
 ...
 */
async function detectStreakLost({ guildId, service = "disboard" }) {
  try {
    const settings = getGuildSettings(guildId) || {};
    const key = `bump_streak_baseline_${service}`;
 ...
 */
async function getBestRankInPeriod(guildId, { periodDays = 7, service = null, bumpsTable = defaultBumpsTable } = {}) {
  try {
    const sb = getSupabase();
    if (!sb) return null;
 ...
 */
function buildStreakLostLine(settings, lostLength) {
 ...
 */
async function runWeeklyMvpTick(client, { bumpsTable = defaultBumpsTable, botName = defaultBotName, nowDate = new Date() } = {}) {
  ...
  for (const guild of client.guilds.cache.values()) {
    try {
      ...
      const { getBumpLeaderboard, getBumpCount } = await getBumpAnalytics();
...
function startWeeklyMvpScheduler(client, opts = {}) {
  ...
  return {
    countBumpsSince,
    maybeCelebrateBumpathon,
    startBumpathonWatcher,
    ...
    buildStreakLostLine,
    runWeeklyMvpTick,
    startWeeklyMvpScheduler,
  };
}

export const _internal = { STREAK_MILESTONES, renderTemplate, _isoWeekKey: _isoWeekKeyShared };
// @ts-nocheck -- shared factory binds untyped Discord/database deps; bot suites cover behavior.
