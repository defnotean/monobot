 ...
 */
/**
 * Build bump/join correlation helpers for one bot.
 *
 * @param {object} deps
 ...
 */
export function createBumpCorrelation({ getSupabase, log = noop, defaultBotName = "eris" } = {}) {
  if (typeof getSupabase !== "function") {
    throw new Error("createBumpCorrelation: getSupabase function is required");
  }

async function recordJoinForCorrelation({ guildId, userId, joinedAtMs = Date.now(), botName = defaultBotName } = {}) {
  ...
  try {
    ...
    if (error) {
      // Suppress "table not found" spam — log once, set a flag to skip future attempts
 ...
 * }
 */
async function getJoinCorrelationStats(guildId, { periodDays = 14, botName = defaultBotName } = {}) {
  const empty = {
    totalJoins: 0,
    postBumpJoins: 0,
    postBumpRatio: 0,
  ...
  return { recordJoinForCorrelation, getJoinCorrelationStats };
}
