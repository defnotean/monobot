// ─── Bump Reminder ──────────────────────────────────────────────────────────
// @ts-nocheck -- shared factory binds untyped Discord/config/database deps; bot suites cover behavior.
 ...
 */
/**
 * Build bump reminder runtime helpers for one bot.
 *
 * @param {object} deps
 * @param {(guildId: string) => any} deps.getGuildSettings
 * @param {(guildId: string, key: string, value: any) => any} deps.setGuildSetting
 * @param {(msg: string) => void} [deps.log]
 * @param {() => Promise<any>} deps.getBumpAnalytics
 * @param {() => Promise<any>} deps.getBumpApplause
 * @param {() => Promise<any>} deps.getBumpCelebrations
 * @param {() => Promise<any>} deps.getBumpUserPrefs
 * @param {() => Promise<any>} deps.getConfig
 * @param {() => any} [deps.getMood]
 * @param {() => Promise<any>} [deps.getPreoccupations]
 * @param {() => Promise<any>} [deps.getPersonality]
 * @param {() => Promise<any>} [deps.getKeyPool]
 * @param {() => Promise<any>} [deps.getGoogleGenAI]
 * @param {(userId: string, amount: number, reason: string, serviceKey: string) => Promise<any>} [deps.updateBalance]
 * @param {string} [deps.defaultBumpsTable]
 * @param {string} [deps.botName]
 * @param {boolean} [deps.logDisabledService]
 * @param {boolean} [deps.logSoftFailures]
 * @param {boolean} [deps.useLeaderboardUsernameFallback]
 * @param {null|undefined} [deps.missingBumperName]
 */
export function createBumpReminder({
  getGuildSettings,
  setGuildSetting,
  log = noop,
  getBumpAnalytics,
  ...
  getPreoccupations = async () => ({}),
  getPersonality = async () => ({}),
  getKeyPool = async () => null,
  getGoogleGenAI = async () => null,
  updateBalance = null,
  defaultBumpsTable = "eris_bumps",
  botName = "eris",
  logDisabledService = true,
  logSoftFailures = true,
  useLeaderboardUsernameFallback = false,
  missingBumperName = undefined,
} = {}) {
  if (typeof getGuildSettings !== "function") throw new Error("createBumpReminder: getGuildSettings function is required");
  if (typeof setGuildSetting !== "function") throw new Error("createBumpReminder: setGuildSetting function is required");
  if (typeof getBumpAnalytics !== "function") throw new Error("createBumpReminder: getBumpAnalytics function is required");
  if (typeof getBumpApplause !== "function") throw new Error("createBumpReminder: getBumpApplause function is required");
  if (typeof getBumpCelebrations !== "function") throw new Error("createBumpReminder: getBumpCelebrations function is required");
  if (typeof getBumpUserPrefs !== "function") throw new Error("createBumpReminder: getBumpUserPrefs function is required");
  if (typeof getConfig !== "function") throw new Error("createBumpReminder: getConfig function is required");

  const maybeLog = (msg) => { if (logSoftFailures) log(msg); };

async function handleBumpConfirm(message, serviceKeyOverride = null) {
  ...
  if (Array.isArray(enabledServices) && !enabledServices.includes(serviceKey)) {
    if (logDisabledService) log(`[BUMP] ${serviceKey} disabled for guild ${guildId} — skipping reminder`);
    return;
  }
  ...
  try {
    ...
    if (bumperId) {
      const { recordBump } = await getBumpAnalytics();
      ...
      try {
        const { sendBumpApplause } = await getBumpApplause();
        const guild = message.client.guilds.cache.get(guildId);
        const member = guild?.members?.cache?.get(bumperId)
          ?? await guild?.members?.fetch?.(bumperId).catch(() => null);
        const bumperName = member?.displayName || member?.user?.username || missingBumperName;
      ...
      try {
        const celebrations = await getBumpCelebrations();
        ...
        await Promise.allSettled([
          celebrations.maybeCelebrateBumpathon(message.client, guildId, defaultBumpsTable),
...
function _schedule(client, guildId, serviceKey, channelId, scheduledAt) {
  ...
  const timer = setTimeout(() => {
    ...
    try {
      ...
      setGuildSetting(guildId, "bump_scheduled_by_service", perService);
    } catch (e) { maybeLog(`[BUMP] ${e.message}`); }
  }, delay);
...
async function _fireReminder(client, guildId, serviceKey, channelId, { wasOffline = false } = {}) {
  ...
  try {
    const { detectStreakLost, buildStreakLostLine } = await getBumpCelebrations();
...
async function fanOutPersonalDms({ guild, serviceKey, svc, body }) {
  try {
    const { getPersonalPingOptIns } = await getBumpUserPrefs();
    const config = await getConfig();
    const optIns = await getPersonalPingOptIns(config.botName || botName);
    ...
    for (const userId of optIns) {
      try {
        ...
        if (!dm) continue;
        const line = botName === "irene"
          ? `**${guild.name}** is bumpable again — \`${svc.slashCommand}\` whenever you've got a sec 🩵\n\n-# you opted into these. turn off with /bumps dm off`
          : `**${guild.name}** is bumpable again — \`${svc.slashCommand}\` when you've got a sec.\n\n-# you opted into these. turn off with /bumps dm off`;
...
async function generateAiReminder({ guild, serviceKey, settings, quiet, wasOffline }) {
  ...
  let streak = 0;
  try {
    const { getBumpLeaderboard, getLastBumper, getGuildStreak } = await getBumpAnalytics();
    ...
    if (leaderboard?.[0]) {
      ...
      topBumperName = m?.displayName || (useLeaderboardUsernameFallback ? leaderboard[0].username : null) || null;
    }
    ...
    streak = await getGuildStreak(guild.id, serviceKey);
  } catch (e) { maybeLog(`[BUMP] ${e.message}`); }
  ...
  try {
    const preoc = await getPreoccupations();
  ...
  try {
    const [config, { getConvClientForBump }] = await Promise.all([
      getConfig(),
...
async function buildRoleMentions(guild, settings, quiet) {
  ...
  if (mode === "rotate") {
    ...
    try { setGuildSetting(guild.id, "bump_rotation_index", nextIdx); } catch (e) { maybeLog(`[BUMP] ${e.message}`); }
...
function shouldSuppressDirectUserPing(userId) {
...
function markDirectUserPinged(userId) {
  if (!userId) return;
...
function scheduleNoShowEscalation({ client, guildId, serviceKey, channelId, sentAt }) {
  ...
  const timer = setTimeout(async () => {
    ...
    try {
      ...
      const { getLastBumper } = await getBumpAnalytics();
...
function cancelEscalation(guildId, serviceKey) {
  const key = keyFor(guildId, serviceKey);
...
async function payFirstBumperBonus(guildId, serviceKey, userId, client) {
  ...
  try {
    if (typeof updateBalance !== "function") return false;
    await updateBalance(userId, FIRST_BUMPER_BONUS_COINS, "bump_first_bonus", serviceKey);
...
function snoozeReminder(guildId, serviceKey, minutes = 15, client = null) {
  const key = keyFor(guildId, serviceKey);
  const existing = _activeTimers.get(key);
  const settings = getGuildSettings(guildId) || {};
  const channelId = settings.bump_reminder_channel_id;
...
// ─── Testing helpers ───────────────────────────────────────────────────────
  return {
    ...
    extractBumperUserId,
    handleBumpConfirm,
    shouldSuppressDirectUserPing,
    markDirectUserPinged,
    cancelEscalation,
    isQuietHoursActive,
    restoreBumpTimers,
    snoozeReminder,
    muteTonight,
    _internal: { SERVICES, keyFor, hourInTimezone, buildCountdownEmbed },
  };
}
// @ts-nocheck -- shared factory binds untyped Discord/config/database deps; bot suites cover behavior.
