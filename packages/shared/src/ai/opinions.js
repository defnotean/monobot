...
// @ts-nocheck -- shared factory binds untyped bot-local personality stores; bot suites cover behavior.
 ...
 *
 * @param {object} deps
 * @param {() => Promise<any>} deps.getData
 * @param {() => void} [deps.markOpinionsDirty]
 * @param {(msg: string) => void} [deps.log]
 * @param {boolean} [deps.safeDates]
 */
export function createOpinions({ getData, markOpinionsDirty, log = noop, safeDates = false } = {}) {
  if (typeof getData !== "function") {
    throw new Error("createOpinions: getData function is required");
  }

async function recordOpinion({ topic, stance, reason = null, strength = 0.5 } = {}) {
 ...
 */
async function findRelatedOpinions(message, limit = 3) {
  const data = await getData();
 ...
 * an old stance.
 */
async function buildOpinionContext(message, opts = {}) {
  ...
  const lines = related.map(op => {
    ...
    const updatedAt = safeDates
      ? (Number.isFinite(parsedUpdated) ? parsedUpdated : now)
      : (op.updatedAt ? Date.parse(op.updatedAt) : now);
    const ageMs = now - updatedAt;
    const ago = safeDates
      ? (op.updatedAt && Number.isFinite(parsedUpdated) ? describeAge(ageMs) : "a while back")
      : (op.updatedAt ? describeAge(ageMs) : "a while back");
    ...
    if (op.flippedAt && op.previousStance) {
      ...
      const flipAge = safeDates
        ? (Number.isFinite(parsedFlip) ? describeAge(now - parsedFlip) : "recently")
        : describeAge(now - parsedFlip);
 ...
 */
async function listRecentOpinions({ topic = null, limit = 10 } = {}) {
  const data = await getData();
  const opinions = Array.isArray(data?.opinions) ? [...data.opinions] : [];
  if (topic) {
