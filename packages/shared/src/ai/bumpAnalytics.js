// ─── Bump Analytics ─────────────────────────────────────────────────────────
// @ts-nocheck -- shared factory binds untyped bot-local Supabase clients; bot suites cover behavior.
...
/**
 * Build bump analytics helpers for a bot-specific bumps table.
 *
 * @param {object} deps
 * @param {() => any} deps.getSupabase
 * @param {(msg: string) => void} [deps.log]
 * @param {string} [deps.bumpsTable]
 */
export function createBumpAnalytics({ getSupabase, log = noop, bumpsTable = "eris_bumps" } = {}) {
  if (typeof getSupabase !== "function") {
    throw new Error("createBumpAnalytics: getSupabase function is required");
  }
 ...
 */
async function recordBump({ guildId, userId, service = "disboard", rank = null, atMs = Date.now() }) {
  if (!guildId || !userId) return { ok: false };
  try {
    const sb = getSupabase();
    if (!sb) return { ok: false, error: "no supabase" };
    const { error } = await sb.from(bumpsTable).insert({
      guild_id: guildId,
      user_id: userId,
      service,
      rank,
 ...
 */
async function getBumpLeaderboard(guildId, { periodDays = null, limit = 10, service = null } = {}) {
  if (!guildId) return [];
  try {
    const sb = getSupabase();
    if (!sb) return [];

    let q = sb.from(bumpsTable).select("user_id, service, bumped_at").eq("guild_id", guildId);
...
async function getLastBumper(guildId, service = null) {
 ...
 */
async function getGuildStreak(guildId, service = null) {
  if (!guildId) return 0;
  try {
    const sb = getSupabase();
    if (!sb) return 0;
    const cutoff = new Date(Date.now() - 60 * 24 * 60 * 60 * 1000).toISOString();
    let q = sb.from(bumpsTable).select("bumped_at").eq("guild_id", guildId).gte("bumped_at", cutoff);
    if (service) q = q.eq("service", service);
...
async function getUserStreak(userId, guildId, service = null) {
  if (!userId || !guildId) return 0;
  try {
    const sb = getSupabase();
    if (!sb) return 0;
    const cutoff = new Date(Date.now() - 60 * 24 * 60 * 60 * 1000).toISOString();
    let q = sb.from(bumpsTable).select("bumped_at").eq("guild_id", guildId).eq("user_id", userId).gte("bumped_at", cutoff);
    if (service) q = q.eq("service", service);
    const { data, error } = await q.order("bumped_at", { ascending: false });
    if (error || !data?.length) return 0;
 ...
 */
async function getBumpCount(userId, guildId, { periodDays = null, service = null } = {}) {
  if (!userId || !guildId) return 0;
  try {
    const sb = getSupabase();
    if (!sb) return 0;
    let q = sb.from(bumpsTable).select("id", { count: "exact", head: true }).eq("guild_id", guildId).eq("user_id", userId);
    if (service) q = q.eq("service", service);
    if (periodDays != null) {
      const cutoff = new Date(Date.now() - periodDays * 24 * 60 * 60 * 1000).toISOString();
      q = q.gte("bumped_at", cutoff);
...
async function getBumpsPerDay(guildId, days = 14, service = null) {
  if (!guildId) return [];
  try {
    const sb = getSupabase();
    if (!sb) return [];
