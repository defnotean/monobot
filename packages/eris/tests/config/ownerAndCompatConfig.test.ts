...
// fallback owner ID — unset/invalid BOT_OWNER_ID fails closed with a loud
// [SECURITY] warning) plus the Phase-1 OpenAI-compat knobs (maxIterations,
// extraBody, toolCoaching) and the turnDeadline timeout.
//
...
describe("config — owner identity fails closed (no hardcoded fallback)", () => {
  ...
  it("ownerId is empty and a [SECURITY] warning fires when BOT_OWNER_ID is unset", async () => {
    const config = await loadConfig();
    expect(config.ownerId).toBe("");
    expect(warnings()).toMatch(/\[SECURITY\] BOT_OWNER_ID is not set/);
    expect(warnings()).toMatch(/owner-only tools are disabled/);
  });

  it("ownerId is empty and warns when BOT_OWNER_ID is not a 17-20 digit snowflake", async () => {
    process.env.BOT_OWNER_ID = "not-a-snowflake";
    const config = await loadConfig();
    expect(config.ownerId).toBe("");
  ...
  it("rejects too-short numeric IDs", async () => {
...
describe("config — Phase-1 OpenAI-compat knobs + turn deadline", () => {
  ...
  it("env overrides win over the local/hosted defaults", async () => {
    ...
    process.env.OPENAI_COMPAT_TOOL_COACHING = "0";
    process.env.OPENAI_COMPAT_COMPACT_SCHEMAS = "0";
  ...
  it("toolCoaching accepts true/1 on hosted providers", async () => {
    process.env.OPENAI_COMPAT_TOOL_COACHING = "true";
    const config = await loadConfig();
    expect(config.openaiCompat.toolCoaching).toBe(true);
  });
