 ...
 */
export async function countBumpsSince(guildId, sinceMs, bumpsTable = "eris_bumps") {
  try {
 ...
 */
export async function maybeCelebrateBumpathon(client, guildId, bumpsTable = "eris_bumps") {
 ...
 */
export function startBumpathonWatcher(client, { intervalMs = 5 * 60_000, bumpsTable = "eris_bumps" } = {}) {
  const tick = async () => {
 ...
 */
export async function getBestRankInPeriod(guildId, { periodDays = 7, service = null, bumpsTable = "eris_bumps" } = {}) {
  try {
    const { getSupabase } = await import("../database.js");
    const sb = getSupabase();
    if (!sb) return null;
 ...
 */
export async function runWeeklyMvpTick(client, { bumpsTable = "eris_bumps", botName = "eris", nowDate = new Date() } = {}) {
