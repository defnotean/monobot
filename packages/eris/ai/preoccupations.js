...
// something else.
//
// Persisted inside the existing eris_personality_learning row under the
// "preoccupation" key — no schema change required.

const ROTATION_MIN_DAYS = 3;
const ROTATION_MAX_DAYS = 8;

// Small curated fallback pool used when no chat signal is strong enough.
// These feel like real late-teens/early-twenties interests rather than
// generic AI safe-talk.
...
let _current = null; // { topic, flavor, source, startedAt, expiresAt, lastInjected }
let _loaded = false;
let _dirty = false;
let _saveTimer = null;
...
async function ensureLoaded() {
  ...
  try {
    ...
    const { data } = await sb.from("eris_personality_learning").select("preoccupation").eq("id", botId).maybeSingle();
...
function scheduleSave() {
  ...
  _saveTimer = setTimeout(async () => {
    ...
    try {
      ...
      if (!sb) return;
      const botId = config.botName || "eris";
      await sb.from("eris_personality_learning").upsert({ id: botId, preoccupation: _current, updated_at: new Date().toISOString() });
      _dirty = false;
    } catch {}
  }, 15_000);
}
...
function pickFromUserStyles(data) {
  ...
  // Only interesting if the top topic has meaningfully more hits than background noise.
...
function pickFromCatchphrases(data) {
  ...
  // Catchphrases can be long — trim aggressively.
...
/**
 * Pick a new preoccupation. Tries to seed from real chat signal first, falls
 * back to the curated pool. `personalityData` is the `_data` object from
 * ai/personality.js (user_styles, server_vibes, catchphrases).
 */
export function pickPreoccupation(personalityData) {
  const roll = Math.random();
  // 40% user-topic, 25% server-vibe, 15% catchphrase, 20% fallback
  const candidates = [];
  if (roll < 0.40) {
...
/**
 * Ensure a current preoccupation exists and is fresh. Rotates when expired.
 * Call this before buildPreoccupationContext at the top of the reply path.
 */
...
/**
 * Returns a system-prompt fragment about what she's been into lately.
 * Injects rarely — preoccupations should feel natural, not constant.
 */
