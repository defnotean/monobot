...
export function trackHumanInteraction(userId, username, message, sentiment, isCreator = false) {
  ...
  // Grudges decay — 1 point per hour, NOT per interaction. The old rule let
  // a toxic user spam-reset their grudge in seconds by flooding messages.
  // Forgiveness should be time-based, not interaction-count based.
  if (rel.grudge > 0 && !isCreator) {
    const now = Date.now();
    const lastDecay = rel._lastGrudgeDecay || 0;
    const hoursSince = (now - lastDecay) / 3_600_000;
    if (hoursSince >= 1) {
  ...
  // Track topics — dedup against the existing list before push so the same
  // word doesn't accumulate a dozen copies before the length cap kicks in.
  ...
  for (const t of topics) {
    if (rel.interests.includes(t)) continue;
    rel.interests.push(t);
    if (rel.interests.length > 10) rel.interests.shift();
  }

  // Streak tracking — use a UTC day integer instead of Date.toDateString()
  // which is locale-sensitive (different runtimes can disagree on the format).
...
export function serialize() {
  ...
  for (const [userId, data] of _relationships) {
    rels[userId] = { ...data };
    delete rels[userId]._lastDay; // Don't persist internal tracking
  }
  return {
    relationships: rels,
    innerState: {
