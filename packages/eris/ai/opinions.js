 ...
 *
 * @param {{ topic?: string, stance?: string, reason?: string|null, strength?: number }} [opinion]
 */
...
export async function buildOpinionContext(message, opts = {}) {
  ...
  const lines = related.map(op => {
    const updatedAt = op.updatedAt ? Date.parse(op.updatedAt) : now;
    const ageMs = now - updatedAt;
    const ago = op.updatedAt ? describeAge(ageMs) : "a while back";
    const reason = op.reason ? ` because ${op.reason}` : "";
    if (ageMs > STALE_OPINION_MS) anyStale = true;
    if (op.flippedAt && op.previousStance) {
      const flipAge = describeAge(now - Date.parse(op.flippedAt));
