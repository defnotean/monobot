...
// OpenRouter free tier was hallucinating: it'd write "ok set that vc as the
// trigger" with no structured tool_calls, and the action never ran.
//
...
describe("Irene TOOL_CALL_DIRECTIVE", () => {
  ...
  it("lists every text-shaped format the model has been observed emitting", () => {
    // The five canonical leaks. If any of these slip through unflagged, the
    // model treats them as valid tool syntax and the action silently fails.
  ...
  it("forbids confirming an action that wasn't actually called this turn", () => {
    // The hallucination mode: bot writes "ok set that vc..." with no tool call.
    // Directive must explicitly veto this so the model says "i tried, retry?"
    // instead of lying.
  ...
  it("forbids prose descriptions of pending tool calls", () => {
    // Models often write "I'll call set_X..." as a stalling pattern. Directive
    // says: just emit the call, don't describe it.
