...
// Smoke tests: web_read must (a) refuse SSRF targets via safeFetch,
...
describe("web_read (irene advancedExecutor)", () => {
  ...
  it("refuses a loopback URL", async () => {
    ...
    const out = await executeAdvanced("web_read", { url: "http://127.0.0.1/admin" }, fakeMessage, fakeCtx);
  ...
  it("refuses file:// URL", async () => {
    const fetchSpy = vi.fn();
    globalThis.fetch = fetchSpy as any;
    const out = await executeAdvanced("web_read", { url: "file:///etc/passwd" }, fakeMessage, fakeCtx);
    expect(String(out)).toMatch(/Failed to read page/i);
    expect(fetchSpy).not.toHaveBeenCalled();
  });
  ...
  it("refuses 169.254.169.254 (cloud metadata)", async () => {
    const fetchSpy = vi.fn();
    globalThis.fetch = fetchSpy as any;
    const out = await executeAdvanced("web_read", { url: "http://169.254.169.254/latest/meta-data/" }, fakeMessage, fakeCtx);
    expect(String(out)).toMatch(/Failed to read page/i);
    expect(fetchSpy).not.toHaveBeenCalled();
  });
  ...
  it("wraps successful output in untrusted envelope", async () => {
    ...
    const out = await executeAdvanced("web_read", { url: "https://1.1.1.1/page" }, fakeMessage, fakeCtx);
