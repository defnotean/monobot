...
- ✅ **Auto-mod rules engine** — Irene can now actually enforce server rules, not just snark at people. Three-tier pipeline: cheap regex pre-filter (NSFW/hate/threat patterns) → Gemini Flash judge with surrounding 8-message context (asked to classify clearly_violates / joking_banter / ambiguous, biased toward inaction) → escalation policy (no auto-bans; ladder caps at 24h timeout). **Off by default** — admins run `/rules enable` to turn on after reviewing extracted rules. New `/rules` slash suite: `learn` (AI-extracts rules from your #rules channel), `list`/`add`/`remove`/`clear` (manage), `enable`/`disable`/`status` (kill switch), `exempt`/`unexempt`/`exemptions` (whitelist users globally or per-rule, optional expiry). Server owner + ManageGuild + ManageMessages holders are always exempt automatically. 36 unit tests on the pre-filter regex + judge prompt + escalation policy.
...
- ✅ **Anti-alt giveaways** — `/giveaway start` gains two optional gates: `min_account_age_days` (Discord account age) and `min_tenure_days` (in this server). Sub-threshold entrants get an ephemeral rejection at button-click time naming which check failed and what the threshold is. Eligibility check + rejection-formatter extracted as a pure module (`utils/giveawayEligibility.js`); 21 unit tests on the gate logic. Gates persist across bot restarts (added to `getGiveawayData`/`initGiveawayData`).
...
- ✅ Anti-alt giveaways (account age / tenure) — shipped 2026-04-25
- ⬜ Threaded reminders
- ⬜ Recurring events
