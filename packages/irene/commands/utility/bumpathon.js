...
export async function execute(interaction) {
  ...
  if (sub === "status") {
    ...
    const { getBumpCount } = await import("../../ai/bumpAnalytics.js");
    // Count bumps since the event started using a rough day window.
    const daysSince = Math.ceil((Date.now() - event.startedAt) / (24 * 60 * 60 * 1000));
    const { getSupabase } = await import("../../database.js");
    let progress = 0;
    const sb = getSupabase();
    if (sb) {
      const { count } = await sb.from("irene_bumps")
