...
const DANGEROUS_PATTERNS = [
  ...
  // Wall-of-text instruction override (500+ chars with jailbreak keywords)
  ...
  // ── Crescendo/Multi-turn softening ──
...
async function checkPatternsAsync(text) {
  ...
  return Promise.race([
    ...
    new Promise(resolve => setTimeout(() => {
      ...
      if (_redosLog.length > 50) _redosLog.shift();
...
// Sync fallback for sliding window checks
...
export async function checkInjection(messageText, supabase, userId) {
  ...
  // Use `config.ownerId` so this file is drop-in with Eris's firewall. Irene
  // exposes both `userId` (legacy) and `ownerId` (alias, same value).
  ...
  // Sliding window (split payloads) — skip on fast path
  addToWindow(userId, messageText);
  if (!isFastPath) {
  ...
  // Semantic check on decoded variants only
