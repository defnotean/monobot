    ...
    if (bumperId) {
      ...
      try {
        ...
        await Promise.allSettled([
          celebrations.maybeCelebrateBumpathon(message.client, guildId, "irene_bumps"),
...
async function _fireReminder(client, guildId, serviceKey, channelId, { wasOffline = false } = {}) {
  ...
  // Personal DM pings — opt-in only, gated by the admin enable flag and
  // deduplicated across servers so users in many guilds don't get pinged
...
async function fanOutPersonalDms({ guild, serviceKey, svc, body }) {
  try {
    ...
    const optIns = await getPersonalPingOptIns(cfg.botName || "irene");
    ...
    for (const userId of optIns) {
      try {
        ...
        if (!dm) continue;
        const line = `**${guild.name}** is bumpable again — \`${svc.slashCommand}\` whenever you've got a sec 🩵\n\n-# you opted into these. turn off with /bumps dm off`;
...
async function generateAiReminder({ guild, serviceKey, settings, quiet, wasOffline }) {
  ...
  // Personality enrichment — her current preoccupation + a real catchphrase
  // so the reminder sounds like HER, not a generic template.
...
async function buildRoleMentions(guild, settings, quiet) {
  ...
  if (mode === "rotate") {
    ...
    try { setGuildSetting(guild.id, "bump_rotation_index", nextIdx); } catch {}
