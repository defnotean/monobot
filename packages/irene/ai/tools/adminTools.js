// @ts-check
// Admin-only Irene AI tool schemas.
// Extracted from ../tools.js; handlers still live in ai/executors/.

import { NEW_ADMIN_TOOLS } from "../newtools.js";

const ROLE_PERMISSION_NAMES = [
  "view_channels", "send_messages", "read_history", "embed_links", "attach_files",
  "add_reactions", "use_ext_emoji", "use_slash_commands", "mention_everyone",
  "manage_messages", "manage_channels", "manage_roles", "manage_guild",
  "kick_members", "ban_members", "timeout_members", "view_audit_log",
  "connect_voice", "speak_voice", "stream", "move_members", "mute_members",
  "deafen_members", "administrator",
];

const CHANNEL_PERMISSION_NAMES = [
  "view", "send", "read_history", "react", "attach", "embed_links",
  "use_ext_emoji", "mention_everyone", "manage_messages", "use_slash",
...
export const ADMIN_TOOLS = [
  ...
  },
  {
    ...
    description: "Set per-channel permission overrides for a user or role. Use allow/deny/inherit arrays with channel permission names; use reset to remove all overrides.",
    input_schema: {
      type: "object",
      properties: {
        channel: { type: "object", description: "Target channel; omit for current.", properties: { name: { type: "string" }, id: { type: "string" } } },
        target: { type: "string", description: "Role name, username, mention, or ID" },
        target_type: { type: "string", enum: ["user", "role"], description: "Whether target names a user or role." },
        reset: { type: "boolean", description: "Remove all overrides for this target." },
        allow: { type: "array", items: { type: "string", enum: CHANNEL_PERMISSION_NAMES }, description: "Permissions to allow." },
        deny: { type: "array", items: { type: "string", enum: CHANNEL_PERMISSION_NAMES }, description: "Permissions to deny." },
        inherit: { type: "array", items: { type: "string", enum: CHANNEL_PERMISSION_NAMES }, description: "Permissions to reset to inherit/default." },
      },
  ...
  // ─── Role Management ────────────────────────────────────────────────
  {
    ...
    description: "Set global server role permissions. Use allow/deny arrays with role permission names; omitted permissions stay unchanged.",
    input_schema: {
      type: "object",
      properties: {
        role_name: { type: "string", description: "Role to edit" },
        allow: { type: "array", items: { type: "string", enum: ROLE_PERMISSION_NAMES }, description: "Permissions to grant." },
        deny: { type: "array", items: { type: "string", enum: ROLE_PERMISSION_NAMES }, description: "Permissions to remove." },
      },
      required: ["role_name"],
    },
  },
  ...
  {
    name: "purge_messages",
    description: "Delete messages with filters for user, content, dates, anchors, media, links, and pins. Large AI purges require confirmation.",
    input_schema: {
      type: "object",
      properties: {
        count: { type: "number", description: "Max messages to scan/delete (1-500, default 100)." },
        channel: { type: "object", description: "Target channel; omit for current.", properties: { name: { type: "string" }, id: { type: "string" } } },
        filters: { type: "object", properties: {
          from_user: { type: "string" }, exclude_user: { type: "string" }, only_keep_media_from_user: { type: "string" },
          content_type: { type: "string", enum: ["all", "media", "text"] }, contains: { type: "string" }, not_contains: { type: "string" },
          has_links: { type: "boolean" }, is_pinned: { type: "boolean" },
        } },
        range: { type: "object", properties: {
          before_message_id: { type: "string" }, after_message_id: { type: "string" },
          before_date: { type: "string" }, after_date: { type: "string" },
        } },
      },
      required: ["count"],
    },
  ...
  },
  {
    ...
    description: "Customize the welcome embed/message. Group text, visual style, media, author/footer, fields, and ping roles; omitted fields stay unchanged.",
    input_schema: {
      type: "object",
      properties: {
        reset: { type: "boolean", description: "Reset all welcome customizations." },
        message: { type: "object", properties: { title: { type: "string" }, title_url: { type: "string" }, description: { type: "string" }, content: { type: "string" }, ping_user: { type: "boolean" } } },
        style: { type: "object", properties: { color: { type: "string" }, show_title: { type: "boolean" }, show_thumbnail: { type: "boolean" }, show_banner: { type: "boolean" }, show_author: { type: "boolean" }, show_footer: { type: "boolean" }, show_timestamp: { type: "boolean" } } },
  ...
  },
  {
    ...
    description: "Only set the naming template for newly created temp VCs. Do not use this to choose or configure the existing join-to-create trigger channel; use set_create_vc_channel for that. Full AVC-compatible template system. Variables: @@creator@@ or {creator} (display name), @@game_name@@ or {game} (game being played, vanishes if none), {game|Fallback Text} or @@game_name|Fallback Text@@ (game, but uses Fallback Text if not playing), @@server_name@@ or {server} (server name), {stream|Fallback Text} (Twitch name if streaming), @@num@@ (user count), @@num_others@@ (others excluding creator), @@nato@@ (NATO phonetic: Alpha/Bravo/...). Numbering: ## (#1, #2...), $# (1, 2...), +# (I, II...), $0# (01, 02...), $00# (001...), $000# (0001...). Singular/plural: <<mouse/mice>> uses @@num@@, <<mouse\\mice>> uses @@num_others@@. Random word: [[Squad/Team/Party]]. Examples: '#1 {game}' -> '#1 Minecraft' (or '#1' if no game) | '{game|Chill Zone}' -> 'Valorant' (or 'Chill Zone' if no game) | '@@nato@@ [[Squad/Team/Party]]' -> 'Alpha Squad'",
    input_schema: {
      type: "object",
      properties: {
  ...
  {
    name: "send_message",
    description: "Send text, an embed, buttons, and/or a role dropdown to a channel. Use components for buttons/dropdowns and embed for rich content.",
    input_schema: {
      type: "object",
      properties: {
  ...
  // ─── Custom Commands ────────────────────────────────────────────────
  {
    ...
    description: "Create a custom !command response. Use embed, roles, and options groups for rich output and behavior controls.",
    input_schema: {
      type: "object",
      properties: {
        trigger: { type: "string", description: "Trigger without !" },
        description: { type: "string", description: "Internal description" },
        response: { type: "string", description: "Text or embed description" },
        embed: { type: "object", properties: { title: { type: "string" }, color: { type: "string" }, url: { type: "string" }, image: { type: "string" }, thumbnail: { type: "string" }, footer: { type: "string" }, author: { type: "string" }, author_icon: { type: "string" } } },
        roles: { type: "object", properties: { give: { type: "string" }, remove: { type: "string" } } },
        options: { type: "object", properties: { admin_only: { type: "boolean" }, auto_delete: { type: "boolean" } } },
      },
  ...
  },
  {
    ...
    description: "Edit an existing custom !command. Use grouped embed, roles, and options fields; omitted values stay unchanged.",
    input_schema: {
      type: "object",
      properties: {
        trigger: { type: "string", description: "Command trigger to edit" },
        description: { type: "string", description: "New internal description" },
        response: { type: "string", description: "New text or embed description" },
        embed: { type: "object", properties: { title: { type: "string" }, color: { type: "string" }, url: { type: "string" }, image: { type: "string" }, thumbnail: { type: "string" }, footer: { type: "string" }, author: { type: "string" }, author_icon: { type: "string" } } },
        roles: { type: "object", properties: { give: { type: "string" }, remove: { type: "string" } } },
        options: { type: "object", properties: { admin_only: { type: "boolean" }, auto_delete: { type: "boolean" } } },
      },
  ...
  // ─── Directives: persistent behavioral rules (admin-only) ─────────────
  // These mutate rules that are injected into Irene's system prompt as
  // admin-set overrides, so they live in ADMIN_TOOLS. This makes dual.js's
  // admin filter, the toolRegistry isAdmin filter, and scheduler.js's
  // isAdminToolName gate all naturally block non-admins (defense in depth on
  // top of the handler gate in ai/executor.js). The read-only counterpart
  // list_directives stays in EVERYONE_TOOLS.
  {
    name: "save_directive",
    description: "Save a behavioral rule/instruction that you will follow persistently. Admin-only. Use when an admin tells you to do (or not do) something as a standing rule. Examples: 'don't reply in #announcements', 'always speak spanish in #español', 'be extra sarcastic in #shitposting'. IMPORTANT: When an admin gives you a standing instruction (not a one-time request), ALWAYS save it as a directive so you remember it forever.",
    input_schema: {
      type: "object",
      properties: {
  ...
  },
  {
    ...
    description: "Remove a saved directive by keyword or index number. Admin-only. Use when an admin says to forget a rule or stop following an instruction.",
    input_schema: {
      type: "object",
      properties: {
  ...
  {
    name: "setup_ticket",
    description: "Configure ticket categories, staff roles, welcome/panel copy, buttons, and ticket types. Omitted fields keep current settings.",
    input_schema: {
      type: "object",
      properties: {
        category: { type: "string", description: "Ticket category name or ID." },
        roles: { type: "object", properties: { view: { type: "array", items: { type: "string" } }, ping: { type: "array", items: { type: "string" } }, view_auto: { type: "string" }, ping_auto: { type: "string" } } },
        welcome: { type: "object", properties: { title: { type: "string" }, description: { type: "string" }, color: { type: "string" } } },
        panel: { type: "object", properties: { title: { type: "string" }, description: { type: "string" }, color: { type: "string" }, channel: { type: "string" }, post: { type: "boolean" }, button: { type: "object", properties: { label: { type: "string" }, emoji: { type: "string" } } } } },
        ticket_types: { type: "array", items: { type: "object", properties: { key: { type: "string" }, label: { type: "string" }, emoji: { type: "string" }, category: { type: "string" }, style: { type: "string" } }, required: ["key", "label"] } },
        remove_ticket_types: { type: "array", items: { type: "string" } },
      },
    },
  },
  {
