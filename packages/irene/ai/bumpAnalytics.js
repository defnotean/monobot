// ─── Bump Analytics ─────────────────────────────────────────────────────────
// Leaderboard + streak + top-bumper queries against the irene_bumps table.
//
// Schema expected:
//   CREATE TABLE irene_bumps (
...
//   );
//   CREATE INDEX irene_bumps_guild_idx ON irene_bumps (guild_id, bumped_at DESC);
//   CREATE INDEX irene_bumps_user_idx  ON irene_bumps (user_id, bumped_at DESC);
//
...
/**
 * @param {{ guildId?: string, userId?: string, service?: string, rank?: number|null, atMs?: number }} args
 */
export async function recordBump({ guildId, userId, service = "disboard", rank = null, atMs = Date.now() }) {
  ...
  try {
    ...
    const { error } = await sb.from("irene_bumps").insert({
      guild_id: guildId,
      user_id: userId,
      service,
      rank,
 ...
 * @returns {Promise<Array<{ user_id: string, count: number, username?: string }>>}
 */
export async function getBumpLeaderboard(guildId, { periodDays = null, limit = 10, service = null } = {}) {
  ...
  try {
    ...
    let q = sb.from("irene_bumps").select("user_id, service, bumped_at").eq("guild_id", guildId);
...
export async function getGuildStreak(guildId, service = null) {
  ...
  try {
    ...
    let q = sb.from("irene_bumps").select("bumped_at").eq("guild_id", guildId).gte("bumped_at", cutoff);
    if (service) q = q.eq("service", service);
...
export async function getUserStreak(userId, guildId, service = null) {
  ...
  try {
    ...
    if (!sb) return 0;
    const cutoff = new Date(Date.now() - 60 * 24 * 60 * 60 * 1000).toISOString();
    let q = sb.from("irene_bumps").select("bumped_at").eq("guild_id", guildId).eq("user_id", userId).gte("bumped_at", cutoff);
    if (service) q = q.eq("service", service);
    const { data, error } = await q.order("bumped_at", { ascending: false });
    if (error || !data?.length) return 0;
...
export async function getBumpCount(userId, guildId, { periodDays = null, service = null } = {}) {
  ...
  try {
    ...
    const sb = getSupabase();
    if (!sb) return 0;
    let q = sb.from("irene_bumps").select("id", { count: "exact", head: true }).eq("guild_id", guildId).eq("user_id", userId);
    if (service) q = q.eq("service", service);
    if (periodDays != null) {
      const cutoff = new Date(Date.now() - periodDays * 24 * 60 * 60 * 1000).toISOString();
      q = q.gte("bumped_at", cutoff);
