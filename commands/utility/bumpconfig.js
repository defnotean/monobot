...
export const data = new SlashCommandBuilder()
  ...
  .addSubcommand(s => s.setName("template")
    .setDescription("Custom reminder message. Vars: {service} {command} {guildName}. Empty = AI voice")
...
export async function execute(interaction) {
  ...
  if (sub === "applause") {
    ...
    return interaction.reply({
      content: enabled
        ? "✅ bump applause on — she'll shout out every bumper"
  ...
  if (sub === "personal_ping") {
    ...
    return interaction.reply({
      content: enabled
        ...
        : "✅ personal DM pings disabled on this server. users will stop receiving DMs from here.",
      ephemeral: true,
    });
  }
  ...
  if (sub === "mvp") {
    ...
    return interaction.reply({
      content: enabled
        ? "✅ weekly MVP DM on — your top bumper will get a thank-you DM every Sunday"
        : "✅ weekly MVP DM off",
      ephemeral: true,
    });
  }
  ...
  if (sub === "celebration_template") {
    ...
    // Preview with sensible sample values so admins see what it'll look like.
