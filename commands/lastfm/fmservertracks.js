// /fmservertracks — Top tracks across all linked server members
...
export const data = new SlashCommandBuilder()
  .setName("fmservertracks")
  .setDescription("Top tracks across all linked members in this server")
...
export async function execute(interaction) {
  ...
  const results   = await getServerTopTracks(linkedIds, limit);

  if (!results.length) {
    return interaction.editReply(
      "no track data indexed yet\n" +
      "-# members need to re-run `/fmset username` to index their tracks"
    );
  }

  const lines = results.map((r, i) =>
    `\`${String(i + 1).padStart(2)}\` **${truncate(r.track_name, 26)}** by ${truncate(r.artist_name, 20)} — ${fmtNum(r.total)} plays · ${r.listeners} listener${r.listeners === 1 ? "" : "s"}`
  );

  const embed = new EmbedBuilder()
    .setColor(FM_COLOR)
    .setTitle(`Top Tracks — ${interaction.guild.name}`)
